            ...
            <div className="flex flex-col max-h-64 overflow-y-auto overflow-x-hidden no-scrollbar">
                {
                    messages.map((message, index) => (
                        <Link to="/messages" key={index} className="flex items-center gap-3 py-3 border-b border-slate-50 last:border-b-0 hover:bg-slate-50 transition-colors px-2 -mx-2 rounded-lg">
                            ...
                            <div className="flex flex-col flex-1 min-w-0">
                                <div className="flex justify-between items-center mb-0.5 min-w-0">
                                    <span className="font-semibold text-slate-900 truncate pr-2 w-full">{message.from_user_id.full_name}</span>
                                </div>
